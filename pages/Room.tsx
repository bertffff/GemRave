...
const RoomPage: React.FC = () => {
  ...
  useEffect(() => {
      if (room?.videoState && videoRef.current) {
          const { isPlaying: remoteIsPlaying, currentTime: remoteTime, timestamp } = room.videoState;
  ...
  // 7. Video Event Handlers (Local Updates)
  const onTimeUpdate = () => {
    if (videoRef.current) {
      setCurrentTime(videoRef.current.currentTime);
      // Optional: Throttle updates to context if needed, but for now we rely on explicit Play/Pause/Seek events
    }
  };
  ...
  const handleSeek = (e: React.MouseEvent<HTMLDivElement>) => {
    if (!progressBarRef.current || !videoRef.current || !id) return;
    ...
    updateVideoState(id, isPlaying, newTime);
  };
        ...
        <div className={`relative flex-1 bg-black flex items-center justify-center transition-all duration-300 ${showChat ? 'h-[40vh] md:h-full md:w-2/3' : 'h-full w-full'}`}>
          ...
          <div className={`absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/90 to-transparent transition-opacity duration-300 ${isPlaying ? 'opacity-0 hover:opacity-100' : 'opacity-100'}`}>
            ...
            <div 
                ref={progressBarRef}
                onClick={handleSeek}
                className="mt-3 w-full h-1 bg-gray-600 rounded-full cursor-pointer hover:h-2 transition-all group/progress relative"
            >
              <div 
                className="h-full bg-blue-500 rounded-full relative transition-all duration-100 ease-linear"
