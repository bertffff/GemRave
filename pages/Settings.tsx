            ...
            <div className="space-y-2">
                ...
                <div className="bg-rave-light/30 rounded-2xl overflow-hidden border border-white/5 backdrop-blur-sm">
                  <SettingItem 
                    ...
                    label="Language" 
                    value={language} 
                    onClick={() => setLanguage(prev => prev === 'English' ? 'Russian' : 'English')}
                    hasArrow 
                  />
                      ...
                      <div className="flex items-center gap-4">
                         ...
                         <div className="flex-1">
                            ...
                            <select 
                                ...
                                className="w-full bg-black/30 border border-white/10 text-sm rounded-xl p-3 text-gray-300 focus:outline-none focus:border-blue-500 transition-colors"
                            >
...
const SettingItem: React.FC<SettingItemProps> = ({ 
    icon, label, value, hasArrow, hasToggle, isChecked, onToggle, onClick 
}) => (
  <div 
    onClick={hasToggle ? onToggle : onClick}
    className="flex items-center justify-between p-4 border-b border-white/5 last:border-0 hover:bg-white/5 active:bg-white/10 cursor-pointer transition-colors"
  >
    <div className="flex items-center gap-4">
    ...
    <div className="flex items-center gap-2">
      ...
      {hasToggle && (
        <div className={`w-11 h-6 rounded-full relative transition-colors duration-200 ease-in-out ${isChecked ? 'bg-blue-600' : 'bg-gray-700'}`}>
           <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-transform duration-200 shadow-sm ${isChecked ? 'left-6' : 'left-1'}`}></div>
        </div>
      )}
    </div>
  </div>
